...
def create_rekall_profile(injector: Injector, file: DLL, raise_on_error=False):
    ...
    while retries <= max_retries:
        try:
            ...
            log.debug(traceback.format_exc())
            log.error(f"Failed to parse json response on {file.path}: {e}")
            ...
            if str(e) == "Some error occurred in injector":
                log.error("Some error occurred in injector")
            else:
        ...
        if retries <= max_retries:
            ...
            time.sleep(1)  # Add a delay before the next retry
