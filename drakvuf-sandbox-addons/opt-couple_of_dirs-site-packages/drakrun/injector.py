...
class Injector:
    ...
    def _run_with_timeout(
        ...
        with subprocess.Popen(args, **kwargs) as proc:
            try:
                ...
                proc.terminate()
                proc.wait(timeout)
                logging.error('Timeout expired while running the command: %s', ' '.join(args))  # Log the error message
            except Exception as e:
                logging.error('An error occurred while running the command: %s', ' '.join(args))  # Log the error message
                logging.exception(e)  # Log the exception traceback
            finally:
                if proc.poll() is None:
                    proc.kill()
            retcode = proc.poll()
            if check and retcode:
                logging.error('Command failed with return code: %s', retcode)  # Log the error message
    ...
    def write_file(
        self, local_path: str, remote_path: str, timeout: int = 180 # increased from 60 for higher success rate
